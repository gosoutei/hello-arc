...
/// @notice Sweeps native USDC from the source wallet to a recipient on Arc.
...
///      Recipient address is prompted in the shell when the script runs.
contract SweepUsdcScript is Script {
    ...
    function run() external {
        address payable recipient = payable(vm.promptAddress("Recipient address")); // 这里就是prompt在shell界面让你输入recipient地址
        require(recipient != address(0), "SweepUsdc: invalid recipient");

        uint256 privateKey = vm.envUint("PRIVATE_KEY");
        address from = vm.addr(privateKey);

        require(from != recipient, "SweepUsdc: sender equals recipient");
